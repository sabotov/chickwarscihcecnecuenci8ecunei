...
{
	public class EffectAnimation : BitActionAnimation
	{
		private static Dictionary<string, GameObject> _effectsCache = new Dictionary<string, GameObject>();

		protected string effname;
		...
		public static GameObject GetEffect(string name)
		{
			...
			{
				_effectsCache.Add(name, WindowLoader.GetPurePref(name, "Prefabs/BattlePrefabs/BattleEffects/"));
			}
			return _effectsCache[name];
		}

		public EffectAnimation(string name)
		{
			effname = name;
			_effect = GetEffect(name);
		}
